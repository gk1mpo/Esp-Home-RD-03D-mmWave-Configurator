...
class EPZoneConfiguratorCard extends HTMLElement {
  ...
  firstUpdated() {
    const canvas = this.shadowRoot.getElementById("visualizationCanvas");
  ...
  initialize() {
    ...
    container.innerHTML = `
    <header class="header">
      <h1>Everything Presence Zone Configurator</h1>
      
    </header>

    <main class="main-content">

      <!-- Canvas wrapper (Option A layout) -->
      <div class="canvas-wrapper">
        <canvas id="visualizationCanvas"></canvas>
      </div>

      <!-- Toolbar area under the canvas -->
      ...
      <!-- Sidebar with zones and settings -->
      <aside class="zone-sidebar">
        <h3>Zones</h3>
        <div class="zone-list"></div>
        <button class="add-zone-button">+ Add Zone</button>
      </aside>
  ...
  `;
    ...
    this.zoneList = this.shadowRoot.querySelector(".zone-list");
    ...
    // Populate devices, attach handlers, etc.
    this.populateDevices();
    // after: this.radarCanvas.bindModel(this.model);
